...
describe("Pagination Utils", () => {
  describe("normalizePaginationParams", () => {
    it("deve usar valores padr\xE3o quando n\xE3o fornecidos", () => {
      const result = normalizePaginationParams(void 0, void 0);
      globalExpect(result.page).toBe(1);
      globalExpect(result.limit).toBe(20);
      globalExpect(result.skip).toBe(0);
    });
    it("deve normalizar valores v\xE1lidos", () => {
      const result = normalizePaginationParams(2, 10);
      globalExpect(result.page).toBe(2);
      globalExpect(result.limit).toBe(10);
      globalExpect(result.skip).toBe(10);
    });
    it("deve usar p\xE1gina padr\xE3o se fornecida inv\xE1lida", () => {
      const result = normalizePaginationParams(0, 10);
      globalExpect(result.page).toBe(1);
    });
    it("deve usar limite padr\xE3o se fornecida inv\xE1lida", () => {
      const result = normalizePaginationParams(1, 0);
      globalExpect(result.limit).toBe(20);
    });
    it("deve limitar m\xE1ximo a 100", () => {
      const result = normalizePaginationParams(1, 200);
      globalExpect(result.limit).toBe(100);
    });
  });
  describe("createPaginationMeta", () => {
    it("deve criar meta corretamente", () => {
      const meta = createPaginationMeta(100, 1, 20);
      globalExpect(meta.total).toBe(100);
      globalExpect(meta.page).toBe(1);
      globalExpect(meta.limit).toBe(20);
      globalExpect(meta.totalPages).toBe(5);
      globalExpect(meta.hasNextPage).toBe(true);
      globalExpect(meta.hasPreviousPage).toBe(false);
    });
    it("deve indicar quando n\xE3o h\xE1 pr\xF3xima p\xE1gina", () => {
      const meta = createPaginationMeta(100, 5, 20);
      globalExpect(meta.hasNextPage).toBe(false);
      globalExpect(meta.hasPreviousPage).toBe(true);
    });
    it("deve calcular total de p\xE1ginas corretamente", () => {
      const meta1 = createPaginationMeta(100, 1, 20);
      globalExpect(meta1.totalPages).toBe(5);
      const meta2 = createPaginationMeta(99, 1, 20);
      globalExpect(meta2.totalPages).toBe(5);
    });
  });
});
